...
export const Util = new class {
	...
	/**
	 * js 复制  放在clipboard error函数里面
	 * @param {dom id} domID 
	 */
	commonCopy(domID){
		var myInput = document.getElementById(domID);
		myInput.select();
		document.execCommand("copy");
		mui.toast('若复制失败，请手动复制！')
	}
	/**
	 * 
...
export const HB = new class {
	...
	/**
	 * 选择相册2  区块链对接限制使用传统input上传选择相册
	 * @param {vue实例} _vue 
	 * @param {回调} callback 
	 */
	picForGallery2(_vue,file,callback){
		let [_this,formData,that] = [_vue,new FormData(),this];
		try{
			Util.uploadImgLimit(file,url=>{
				formData.append('image',url);
				Base.upload(formData).then((res)=>{
				Load.loadEnd(_this);
					if(res.code == 1){
		...
		try{
			...
			plus.downloader.createDownload( _this.wgtUrl, {filename:"_doc/update/"}, function(d,status){  
				if ( status == 200 ) {   
					...
					plus.nativeUI.confirm("下载资源文件失败,点击[确定]重新下载更新包",(e)=>{
						if(e.index ==0){
							_this.download();
	...
	installWgt(path){  
		...
		try{
			...
			plus.runtime.install(path,{},function(){  
				...
				// if(e.code == '-9999'){
					plus.nativeUI.confirm("更新资源失败["+e.code+"]："+e.message+'点击[确定]重新下载更新包',(e)=>{
						if(e.index ==0){
							_this.download();
						}
					})
...
export const Pay = new class {
	constructor(){
        this.ALIPAYSERVER = store.state.API+'/addons/litestore/api.order/alipay_pay';//支付宝支付
        this.WXPAYSERVER = store.state.API +'/addons/litestore/api.order/wechat_pay';//微信支付
