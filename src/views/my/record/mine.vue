<template>
    <div class="page">
        <m-header title="抢购记录" :canback="Boolean(1)"></m-header>
        <section class="body">
            ...
            <ul :style="'height:'+(screenHeight-100)+'px;overflow:auto;'" v-lazy-load="lazyLoad">
                <div :class="{'record':true}" v-for="(item,index) in listData" :key="index">
                    ...
                    <div class="btn-groug">
                        ...
                        <button class="btn btn-detail btn-blue" v-show="mavIndex == 1"  @click="delore(item.id,item.pcp)">销毁</button>
                    </div>
                </div>
...
export default {
    ...
    methods:{
        ...
        delore(id,pic){
            let _this = this;
            let money = Number(+pic*this.per + +pic/(+this.price)).toFixed(2)
            function dl(){
                Owner.delore({id}).then(res=>{
                    mui.toast(res.msg)
                    if(res.code==1){
                        _this.change(_this.value)
                    }
                })
            }
            try{
                if(plus){
                    mui.confirm('当前宠物销毁后可获得的MGK数量为'+money+'个',actions=>{
                        if(actions.index==0)
                        dl();
                    })
                }
        ...
        change(value){
            // console.log(value)
            [this.page,this.listData,this.noMore,this.value] = [1,[],false,value];
           switch(value){
               case 0:
                    this.mavIndex = 0;
                    this.detailType = 0;
