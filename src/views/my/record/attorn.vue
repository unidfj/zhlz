            ...
            <ul :style="'height:'+(screenHeight-100)+'px;overflow:auto;'" v-lazy-load="lazyLoad">
                <div :class="{'record':true}" v-for="(item,index) in listData" :key="index">
                    ...
                    <div class="btn-groug">
                        ...
                        <button class="btn btn-detail btn-blue" @click="delore(item.id,item.total_money)" v-show="mavIndex == 0">销毁</button>
                    </div>
                </div>
...
export default {
    ...
    methods:{
         ...
         */
        navChange(index){
            [this.page,this.listData,this.noMore,this.value] = [1,[],false,index];
            switch(index){
                case 0:
                    this.mavIndex = 0;
                    this.detailType = 0;
